    ...
    <div className="min-h-screen bg-deepBlue bg-opacity-95 text-white overflow-x-hidden">
      ...
      <nav className="fixed w-full z-50 bg-background/90 backdrop-blur-sm">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          <div className="text-2xl font-bold text-neonBlue animate-glow">Subhs</div>
          <div className="flex gap-8">
            {['Home', 'About', 'Portfolio', 'Contact'].map((item) => (
              <button
                key={item}
                onClick={() => scrollToSection(item.toLowerCase())}
                className="bg-blue-gradient bg-clip-text text-transparent hover:animate-glow transition-all"
              >
                {item}
              </button>
            ))}
      ...
      <section id="home" className="min-h-screen flex items-center justify-center pt-20">
        <div className="container mx-auto px-6" data-aos="fade-up">
          <div className="max-w-4xl mx-auto">
            <div className="text-neonBlue mb-8 inline-block px-6 py-2 rounded-full border border-neonBlue animate-glow">
              Ready to Innovate
            </div>
            <h1 className="text-6xl font-bold mb-6 text-neonBlue animate-glow">Frontend<br/>Developer</h1>
            <h2 className="text-3xl mb-4 text-lightBlue">Network &</h2>
      ...
      <section id="about" className="min-h-screen flex items-center justify-center py-20">
        <div className="container mx-auto px-6" data-aos="fade-up">
          <h2 className="text-4xl font-bold text-center mb-12 text-neonBlue animate-glow">About Me</h2>
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="flex gap-4">
                ...
                <button onClick={() => scrollToSection('portfolio')} className="flex items-center gap-2 border border-neonBlue px-6 py-3 rounded-lg hover:bg-neonBlue/20 transition-all">
                  <Code size={20} />
                  View Projects
                </button>
              </div>
      ...
      <section id="portfolio" className="min-h-screen py-20">
        <div className="container mx-auto px-6" data-aos="fade-up">
          <h2 className="text-4xl font-bold text-center mb-12 text-neonBlue animate-glow">Portfolio Showcase</h2>
          ...
          <div className="flex justify-center gap-6 mb-12">
            {[
              { title: 'Projects', icon: <Code size={20} /> },
              { title: 'Certificates', icon: <Award size={20} /> },
              { title: 'Tech Stack', icon: <FileText size={20} /> }
            ].map((item) => (
              <button 
                key={item.title}
                onClick={() => scrollToSection(item.title.toLowerCase())}
                className="flex items-center gap-2 px-8 py-4 rounded-lg bg-blue-gradient hover:opacity-90 transition-all text-white"
              >
                {item.icon}
                {item.title}
              </button>
          ...
          <div id="tech-stack" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {techStack.map((tech) => (
              <div key={tech.name} className="p-6 rounded-lg bg-deepBlue border border-neonBlue/20 hover:border-neonBlue transition-all">
